...
function HomeMixEditor({ homes, onUpdate, onAdd, onRemove, wage, setWage, rates = RATES_DEF, setRates, graveyardWage, setGraveyardWage, occupancy, setOccupancy, canEdit = true, canAdd = canEdit, userRole }) {
  ...
  const prevCount = useRef(homes.length);
  useEffect(()=>{
...
const SUB_TABS = {
  ...
  CHILDRENS_DDA: [
    ...
    { id: "chdda_sandbox",          label: "🔬 Sandbox" },
        ...
        <div style={{ display:"grid", gridTemplateColumns:"240px 1fr", flex:1, minHeight:0 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", overflow:"hidden", background:"#f5f7fa" }}>
            ...
            <div style={{ padding:"20px 24px", overflowY:"auto", flex:1 }}>
              ...
              {/* CHILDRENS_DDA tabs */}
              {activeSLType === SERVICE_LINE_TYPES.CHILDRENS_DDA && activeSL && subTab === "chdda_current_services" && (
                <ChildrensDDACurrentServicesTab config={activeSL.config}
                  onUpdate={cfg => updateServiceLineConfig(activeSL.id, cfg)}
                  userRole={userRole}/>
              )}
              {activeSLType === SERVICE_LINE_TYPES.CHILDRENS_DDA && activeSL && subTab === "chdda_sandbox" && canEditServiceLines(userRole) && (
                <ChildrensDDASandboxTab config={activeSL.config}
                  onUpdate={cfg => updateServiceLineConfig(activeSL.id, cfg)}
                  userRole={userRole}/>
              )}
